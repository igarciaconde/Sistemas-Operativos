...
static int my_unlink(const char *path) {
	...
	// comprobar que la longitud del nombre es correcta
	if (strlen(path + 1) > myFileSystem.superBlock.maxLenFileName) {
		return -ENAMETOOLONG;
	}

	int idxDir, idxBlocks, i;
	// comprobamos que no exista

	if ((idxDir = findFileByName(&myFileSystem, (char *)path + 1)) == -1)	// == instead of !=
		return -EEXIST;

	//Recorremos los bloques dónde está el contenido del nodo-i
	for (i = 0; i< myFileSystem.nodes[idxDir]->numBlocks; i++)
	{
		idxBlocks = myFileSystem.nodes[idxDir]->blocks[i];
		myFileSystem.bitMap[idxBlocks] = 0;
	}
	...
	//Actualizo el numero de bloques en SuperBlock
	myFileSystem.superBlock.numOfFreeBlocks = myQuota(&myFileSystem);

	//Creo un nodo libre y lo situo en los nodos-i
	NodeStruct currentNode;
	currentNode.freeNode = true;

	//actualizamos el nodo el superbloque el bitmap y lo que resta
	updateSuperBlock(&myFileSystem);
	updateBitmap(&myFileSystem);
	updateDirectory(&myFileSystem);
	updateNode(&myFileSystem, idxDir, &currentNode);
...
static int my_read(const char* path, char* buf, size_t size, off_t offset, struct fuse_file_info* fi) {
	...
	while (bytes2Read) {
		...
		//comprobamos que no nos hemos salido del bloque ni que la lectura haya fallado
...
struct fuse_operations myFS_operations = {
	...
	.read		= my_read,						//Read an opened file
	.unlink		= my_unlink,					
};
